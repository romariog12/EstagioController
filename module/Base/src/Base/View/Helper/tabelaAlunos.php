 ...
 **/
class tabelaAlunos extends AbstractHelper{
    protected $td;
    protected $th;
    ...
    public function tdTh($valueTh = [],$valueTd = [], $numeroPagina = []){
        ...
        foreach ($valueTd as $l){
            ...
            $html   .= '<td><a href="'.$this->view->url(Constantes::rotaAdministradorDefault, ['controller'=>Constantes::administrador, 'action'=>  Constantes::editarAluno, 'id'=>$l[0]->getIdaluno(),'page'=>$numeroPagina]).'"><i class=" glyphicon glyphicon-edit"></i></a></td>';
            $html   .= '<td><a onclick="return confirm("Tem certeza que deseja deletar este registro?")" style="color: #c81616" href="'.$this->view->url(Constantes::excluir,['action'=>  Constantes::excluirAluno, 'id'=>$l[0]->getIdaluno(),'page'=>$numeroPagina]).'"><i  class=" glyphicon glyphicon-remove"></i></a></td>';
            $html   .= '</tr>';
        }
        return $html;
    }
