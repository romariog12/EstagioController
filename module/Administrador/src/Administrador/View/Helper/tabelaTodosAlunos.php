...
class tabelaTodosAlunos extends AbstractHelper{
    ...
    public function tdTh($valueTh = [],$valueTd = [], $numeroPagina = []){
        ...
        foreach ($valueTd as $l){
            ...
            $html   .= '<td><a onclick="return confirm("Tem certeza que deseja deletar este registro?")" style="color: #c81616" href="'.$this->view->url('excluirAluno',['action'=>'excluirAlunoPresencial', 'deleteAluno'=>$l->getIdaluno(),'page'=>$numeroPagina]).'"><i  class=" glyphicon glyphicon-remove"></i></a></td>';
            $html   .= '</tr>';
        }
        return $html;
    }
